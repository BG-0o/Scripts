...
local function ResetUniversalRuntimeState()
    local env = getgenv()

    LoopFlingInput = nil
    LoopFlingGeneration += 1

    -- Stop every external/game module that exposes a cleanup handler.
    local cleanupNames = {
        "ToxUniversal2Cleanup",
        "ToxMM2Cleanup",
        "ToxNDSCleanup",
        "ToxSystemsCleanup",
        "ToxADMINCleanup",
    ...
    for _, cleanupName in ipairs(cleanupNames) do
        local cleanup = env[cleanupName]
    ...
    if env.SetNDSWaterFly then
        pcall(function()
            env.SetNDSWaterFly(false, true)
        end)
    end
    ...
    -- Central rule: every boolean option is OFF on shutdown/re-execution.
    -- Render3D is restored to true because that is Roblox's normal state.
    ...
    local sharedKeys = env.SharedPersistentKeys or {}
    local baseShared = env.BaseSharedSettings or {}
    ...
    pcall(StopWalkFling)
    pcall(StopAntiVoid)
    pcall(DestroyCarFlyMovers)
    pcall(RestoreSpeed)
    pcall(RestoreJump)
    pcall(RestoreNoclipDefaults)
    pcall(RestoreAntiFlingDefaults)
    pcall(RestoreHitboxDefaults)
    pcall(RestoreFOVDefault)
    pcall(RestoreShiftLockDefaults)
    ...
    if env.ActiveSound then
        pcall(function()
            env.ActiveSound:Stop()
            env.ActiveSound:Destroy()
        end)
...
getgenv().ToxUniversalCleanup = ResetUniversalRuntimeState
...
CreateConfirmButton("DESTROY", ConfigPage, function()
    ...
    -- Keep saved non-boolean configuration, but force the next execution to
    -- start with every runtime toggle disabled.
    getgenv().ToxForceFreshToggleState = true
    ...
    -- One centralized restoration pass for Universal + game modules.
    if getgenv().ToxUniversalCleanup then
        pcall(getgenv().ToxUniversalCleanup)
    end
