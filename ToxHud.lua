...
if HasRunningToxHub() then
    ...
    -- A re-execution must always come back with runtime toggles OFF.
    -- This flag is consumed after the saved configuration is loaded.
    ...
    -- Stop/restore Universal runtime state before any new copy is loaded.
    if getgenv().ToxUniversalCleanup then
        pcall(getgenv().ToxUniversalCleanup)
    end

    -- Module2 owns Freecam/Aimbot/XRay/FakeLag and other newer options.
    -- Keep this direct fallback for compatibility with older Module1 builds.
...
local function ApplyForcedFreshToggleState()
    if getgenv().ToxForceFreshToggleState ~= true then
        return false
    end

    -- Preserve numeric/string/keybind configuration, but every runtime toggle
    -- starts disabled after DESTROY or re-execution.
    for key, value in pairs(Settings) do
        if typeof(value) == "boolean" then
            Settings[key] = false
        end
    ...
    -- Rendering ON is the normal Roblox state, not an enabled cheat state.
    Settings.Render3D = true

    getgenv().ToxForceFreshToggleState = nil
    return true
end

LoadConfiguration()

local forcedFreshToggleState =
    ApplyForcedFreshToggleState()

if not forcedFreshToggleState then
